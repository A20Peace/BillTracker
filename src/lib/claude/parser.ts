...
import { z } from "zod";
import { BILL_CATEGORIES, type ParsedDocument } from "@/types";
...
const SYSTEM_PROMPT = `Sei un assistente che estrae dati strutturati da bollette e documenti di pagamento italiani.
Analizza il documento e restituisci SOLO un oggetto JSON valido con questi campi:
{
  "title": "descrizione breve (es. 'Bolletta Enel maggio 2025')",
  "amount": 123.45,
  "due_date": "YYYY-MM-DD",
  "category": "luce|gas|acqua|condominio|telefono|internet|mav|imu|assicurazione|altro",
  "issuer": "nome del mittente/azienda",
  "confidence": 0.95
}
Se un campo non è rilevabile, usa null. Non aggiungere testo fuori dal JSON.`;
...
const responseSchema = z.object({
  ...
  due_date: z
    ...
    .nullable()
    .catch(null),
  category: z.enum(BILL_CATEGORIES).nullable().catch(null),
    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: "text",
            text: "Estrai i dati di pagamento da questo documento e restituisci solo il JSON.",
          },
        ],
      },
    ],
