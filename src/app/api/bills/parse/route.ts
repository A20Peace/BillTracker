 ...
 * Claude, and returns them for user confirmation before saving.
 */
export async function POST(request: NextRequest) {
  ...
  // respond clearly in case the endpoint is still called.
  ...
  try {
    ...
    // 1) Persist the original document under the user's folder.
    ...
    // Log only a generic message — never the document or extracted contents.
    console.error("[bills/parse] parsing failed:", message);
    return NextResponse.json<ApiError>(
